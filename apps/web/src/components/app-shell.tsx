...
function WorkspaceSwitcher({
  ...
  const selectedIndex = selectedWorkspace ? workspaces.indexOf(selectedWorkspace) : 0;
  ...
  return (
    <div className="workspace-switcher" ref={rootRef}>
      ...
      >
        <span className="workspace-avatar" aria-hidden="true">{selectedWorkspace?.name.slice(0, 1).toUpperCase() || "A"}</span>
        <span className="workspace-trigger-copy">
          <strong>{selectedWorkspace?.name || "Select workspace"}</strong>
          <span>{selectedWorkspace ? `${selectedWorkspace.role === "OWNER" ? "Owner" : "Member"} workspace` : "No workspace yet"}</span>
        </span>
      ...
      {open ? (
        <div className="workspace-menu" role="listbox" aria-label="Workspaces">
          {workspaces.length === 0 ? <div className="workspace-empty">No workspaces yet. Open Settings to create one.</div> : null}
          {workspaces.map((workspace, index) => (
            <button
              className={workspace.id === selectedWorkspaceId ? "workspace-option selected" : "workspace-option"}
              key={workspace.id}
              type="button"
              role="option"
              ...
              onKeyDown={(event) => {
                ...
                if (event.key === "End") { event.preventDefault(); focusOption(workspaces.length - 1); }
                if (event.key === "Escape") setOpen(false);
              }}
            >
              <span className="workspace-option-avatar" aria-hidden="true">{workspace.name.slice(0, 1).toUpperCase()}</span>
              <span className="workspace-option-copy"><strong>{workspace.name}</strong><small>{workspace.role === "OWNER" ? "Owner" : "Member"}</small></span>
              {workspace.id === selectedWorkspaceId ? <span className="workspace-option-check" aria-hidden="true">✓</span> : null}
            </button>
          ))}
        </div>
      ) : null}
      ...
      <aside className={sidebarCollapsed ? "sidebar collapsed" : "sidebar"} data-mobile-open={mobileOpen}>
        ...
        <WorkspaceSwitcher
          ...
          onChange={(workspaceId) => {
            ...
            window.localStorage.setItem("vf-workspace-id", workspaceId);
          }}
        />
      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="avatar-button" type="button" aria-label={`Signed in as ${email || "user"}`}>{email ? email.slice(0, 2).toUpperCase() : "VF"}</button>
          </div>
        </header>
