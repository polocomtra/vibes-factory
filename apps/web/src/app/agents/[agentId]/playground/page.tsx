...
function groupConversation(messages: Message[]): ConversationItem[] {
    ...
    for (const message of messages) {
        if (
            ...
            message.content.type === "tool_calls"
        ) {
        ...
        if (message.role === "TOOL") {
            const toolCallId =
                typeof message.content.tool_call_id === "string"
...
function toolResultLabel(tool: ToolCallRecord) {
    ...
    if (
        isRecord(tool.result.output) &&
        Array.isArray(tool.result.output.results)
    ) {
...
function ToolActivityGroup({ tools }: { tools: ToolCallRecord[] }) {
    const groups = tools.reduce<
        Array<{ name: string; calls: ToolCallRecord[] }>
    >((accumulator, tool) => {
    ...
    return (
        <section className="inline-tool-activity" aria-label="Tool activity">
            ...
            <div className="tool-activity-groups">
                {groups.map((group) => {
                    const failedCount = group.calls.filter(
                        (call) => call.result && !call.result.ok,
                    ).length;
                    return (
                        <details
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="tool-call-meta">
                                                ...
                                                >
                                                    {toolResultLabel(tool)}
                                                </span>
                                            </div>
                                            {tool.result && !tool.result.ok ? (
                                                <p className="tool-call-output error">
                                                    {toolResultError(tool)}
                                                </p>
                                            ) : null}
                                        </div>
                                    );
...
const markdownComponents: Components = {
    ...
    pre: ({ children }) => {
        ...
        const codeElement = isValidElement(child)
        ...
        return (
            <div className="markdown-code-block">
                <div className="markdown-code-toolbar">
                    <span>{language}</span>
                    <CopyButton text={code} label={`Copy ${language} block`} />
                </div>
                <pre>
                    <code className={codeElement?.props.className}>{code}</code>
                </pre>
            </div>
        );
    },
...
function PlaygroundMessage({
    ...
    return (
        ...
        >
            <article
                className={`playground-message ${message.role.toLowerCase()}`}
            >
            ...
            {tools.length > 0 ? <ToolActivityGroup tools={tools} /> : null}
...
function TraceInspector({
    ...
    async function selectSpan(span: Span) {
        setSelected(span);
        try {
            setSelected(await fetchSpan(span.id));
        } catch (reason: unknown) {
            setError(
                reason instanceof Error
                    ? reason.message
            ...
            >
                <header className="trace-inspector-header">
                    <div>
                        ...
                        <h2 id="trace-title">Run trace</h2>
                        <p className="panel-copy">
                            Inspect the persisted execution timeline for{" "}
                    ...
                    <div className="trace-split-view">
                        ...
                        >
                            <div className="trace-summary">
                                <StatusBadge status={trace.trace.status} />
                            ...
                            {spans.map((span) => (
                                <button
                                    ...
                                    type="button"
                                    key={span.id}
                                    onClick={() => void selectSpan(span)}
                                >
                                    <span
                                        className={`trace-span-icon ${span.type.toLowerCase()}`}
                                        aria-hidden="true"
                                    >
                                        {span.type === "MODEL" ? (
                                            <Bot size={14} />
                                        ) : span.type === "CONTEXT_BUILD" ? (
                                            <FileText size={14} />
                                        ) : span.type === "TOOL" ? (
                                            <SearchIcon size={14} />
                                        ) : span.type === "RETRIEVAL" ? (
                            ...
                            {selected ? (
                                <>
                                    ...
                                    >
                                        ...
                                        <div className="trace-usage-grid">
                                            <span>
                                                <small>Input</small>
                                                <b>
                                                    {tokenLabel(
                                ...
                                <div className="trace-empty">
                                    Select a span to inspect its payload.
                                </div>
                            )}
                        </div>
                    </div>
...
export default function PlaygroundPage() {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                const [agentData, versionData, sessionData] = await Promise.all(
                    [
                        fetchAgent(agentId),
                ...
                const selected =
                    sessionData.find((item) => item.id === querySession) ??
                    (querySession ? null : sessionData[0]) ??
                    null;
                if (selected) {
    ...
    return (
        <AppShell>
            <div className="playground-page">
                        ...
                        <div className="playground-title">
                            ...
                            <div>
                                ...
                                <p className="page-description">
                                    Run a published immutable version against a
                                    persisted session.
                                </p>
                            </div>
                        </div>
                ...
                {error ? (
                    <div className="form-error playground-alert" role="alert">
                        <CircleAlert size={15} aria-hidden="true" />
                        {error}
                        {lastRun?.id && lastRun.trace_id ? (
                            <button
                                className="text-button"
                                type="button"
                                onClick={() => setTraceRunId(lastRun.id)}
                            >
                                Open failed trace
                            </button>
                        ) : null}
                ...
                {!selectedVersion ? (
                    <section className="panel agent-empty-state playground-empty">
                        <GitBranch size={28} aria-hidden="true" />
                        <h2>Publish a version to run this agent</h2>
                        <p className="panel-copy">
                            The playground never executes mutable draft state.
                            Publish an immutable version from the agent
                            configuration first.
                        </p>
                        <button
                    ...
                    <section className="panel agent-empty-state playground-empty">
                        ...
                        <h2>Start a playground session</h2>
                        <p className="panel-copy">
                            Create a session to persist this conversation and
                            its runtime traces.
                        </p>
                        <button
                            className="button primary-button"
                                ...
                                <div className="run-status-area">
                                    ...
                                    {lastRun?.usage?.total_tokens ? (
                                        <span className="token-summary">
                                            {lastRun.usage.total_tokens.toLocaleString()}{" "}
                                            tokens
                                        </span>
                                    ) : null}
                                </div>
                ...
                {messages.length === 0 && !busy ? (
                                ...
                                {toolActivity ? (
                                    <div
                                        className="tool-runtime-panel tool-runtime-live"
                                        role="status"
                                    >
                                        <LoaderCircle
                                            className="spin"
                                ...
                                {streamingText ? (
                                    ...
                                    >
                                        ...
                                        <MessageContent
                                            message={{
                                                id: `streaming-${streamRunId ?? "pending"}`,
                                                session_id: sessionId,
                                                run_id: streamRunId,
                                                role: "ASSISTANT",
                                                sequence_no:
                            ...
                            <div className="composer-wrap">
                                <label htmlFor="playground-composer">
                                    Message
                                </label>
                        ...
                        <aside className="playground-side-column">
                            ...
                            <section
                                className="panel recent-run-panel"
                                aria-busy={tracesLoading}
                            >
                                <div className="panel-heading">
                                    <div>
                                        <span className="panel-kicker">
                                        ...
                                        <h2>Session runs</h2>
                                        <p className="panel-copy">
                                            Trace and span history for this
                                            session.
                                        </p>
                                    </div>
                                    <span className="session-trace-count">
                                ...
                                {tracesLoading ? (
                                    ...
                                    <p className="panel-copy">
                                        Your first run will appear here with its
                                        trace and spans.
                                    </p>
                                )}
                            </section>
