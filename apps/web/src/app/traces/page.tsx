...
import { Bot, CalendarDays, Check, CircleAlert, Grid2X2, List, LoaderCircle, RotateCcw, Search, SlidersHorizontal } from "lucide-react";
...
function statusTone(status: TraceListItem["status"]) {
  return status === "COMPLETED" ? "success" : status === "FAILED" ? "error" : "info";
}

function TraceStatus({ status }: { status: TraceListItem["status"] }) {
  return <span className={`status-badge ${statusTone(status)}`}><span aria-hidden="true" />{status.replaceAll("_", " ")}</span>;
}
...
function TracePreview({ trace }: { trace: TraceSessionItem }) {
  const hasDifferentMessages = trace.first_user_message !== trace.latest_user_message;
  return <div className="trace-message-summary"><div><small>First user message</small><p>{trace.first_user_message || "No input recorded."}</p></div>{hasDifferentMessages ? <div><small>Latest user message</small><p>{trace.latest_user_message || "No input recorded."}</p></div> : null}</div>;
}

function TraceGridCard({ trace, onOpenSession }: { trace: TraceSessionItem; onOpenSession: (trace: TraceSessionItem) => void }) {
  return <article className="trace-card"><div className="trace-card-topline"><span className="trace-agent-mark"><Bot size={15} aria-hidden="true" /></span><TraceStatus status={trace.status} /></div><div className="trace-card-heading"><div><h2>{trace.agent_name}</h2><p>{trace.trace_count} {trace.trace_count === 1 ? "run" : "runs"} · <code>{trace.run_id.slice(0, 10)}…</code></p></div><span className="trace-duration">{durationLabel(trace.duration_ms)}</span></div><TracePreview trace={trace} /><div className="trace-card-meta"><span><small>Latest activity</small><b>{dateLabel(trace.started_at)}</b></span><span><small>Version ID</small><b>{trace.agent_version_id.slice(0, 8)}…</b></span></div><footer><code>{trace.session_id ? `Session ${trace.session_id}` : `Trace ${trace.id}`}</code>{trace.session_id ? <button className="text-button" type="button" onClick={() => onOpenSession(trace)}>Open session <span aria-hidden="true">→</span></button> : <span className="trace-no-session">No session</span>}</footer></article>;
}

function TraceListRow({ trace, onOpenSession }: { trace: TraceSessionItem; onOpenSession: (trace: TraceSessionItem) => void }) {
  return <article className="trace-list-row"><div className="trace-list-agent"><span className="trace-agent-mark"><Bot size={15} aria-hidden="true" /></span><span><strong>{trace.agent_name}</strong><small>{trace.trace_count} {trace.trace_count === 1 ? "run" : "runs"} · <code>{trace.run_id.slice(0, 10)}…</code></small></span></div><TraceStatus status={trace.status} /><span className="trace-list-time"><CalendarDays size={14} aria-hidden="true" />{dateLabel(trace.started_at)}</span><span className="trace-list-duration">{durationLabel(trace.duration_ms)}</span><code className="trace-list-id">{trace.session_id ? trace.session_id.slice(0, 12) : trace.id.slice(0, 12)}…</code>{trace.session_id ? <button className="button secondary-button trace-open-button" type="button" onClick={() => onOpenSession(trace)}>Open session</button> : <span className="trace-no-session">No session</span>}</article>;
}

function groupTracesBySession(traces: TraceListItem[]): TraceSessionItem[] {
  const groups = new Map<string, TraceSessionItem>();
  const chronological = [...traces].sort((left, right) => Date.parse(left.started_at) - Date.parse(right.started_at));
  for (const trace of chronological) {
    const key = trace.session_id ?? `trace:${trace.id}`;
    const existing = groups.get(key);
    if (!existing) {
      const { input_text, ...latestTrace } = trace;
      groups.set(key, { ...latestTrace, first_user_message: input_text, latest_user_message: input_text, trace_count: 1 });
      continue;
    }
    existing.trace_count += 1;
    if (!existing.first_user_message && trace.input_text) existing.first_user_message = trace.input_text;
  ...
  return [...groups.values()].sort((left, right) => Date.parse(right.started_at) - Date.parse(left.started_at));
}
...
export default function TracesPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setError(null);
      setNextCursor(null);
      void fetchTraces({ agent: agentSearch.trim() || undefined, status: status || undefined, started_after: rangeStart(timeRange) })
        .then((page) => { if (!cancelled) { setTraces(page.data); setNextCursor(page.pagination.next_cursor); } })
        .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "Unable to load traces."); })
  ...
  async function loadMore() {
    ...
    setError(null);
    try {
      const page = await fetchTraces({ agent: agentSearch.trim() || undefined, status: status || undefined, started_after: rangeStart(timeRange) }, nextCursor);
      setTraces((items) => [...items, ...page.data]);
      setNextCursor(page.pagination.next_cursor);
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : "Unable to load more traces.");
    } finally {
  ...
  const summary = useMemo(() => ({
    ...
    completed: sessionItems.filter((trace) => trace.status === "COMPLETED").length,
    failed: sessionItems.filter((trace) => trace.status === "FAILED").length,
  }), [sessionItems]);
