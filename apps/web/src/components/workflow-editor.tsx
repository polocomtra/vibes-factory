...
import { Braces, Check, ChevronDown, CircleAlert, Database, GitBranch, Info, LayoutGrid, PanelRightClose, PanelRightOpen, Play, Plus, Save, Search, Settings2, ShieldCheck, Sparkles, Trash2, X } from "lucide-react";
...
function WorkflowNode({ data, selected }: NodeProps<CanvasNode>) {
  return <div className={`workflow-canvas-node ${selected ? "selected" : ""}`} style={{ "--node-accent": colors[data.type] } as React.CSSProperties} tabIndex={0} aria-label={`${typeLabels[data.type]} node: ${data.label}`}>
    ...
    {data.type === "CONDITION" ? <><Handle id="true" type="source" position={Position.Right} style={{ top: "35%" }} /><Handle id="false" type="source" position={Position.Right} style={{ top: "70%" }} /></> : data.type !== "END" ? <Handle type="source" position={Position.Right} /> : null}
  </div>;
}
...
export function WorkflowEditor({ workflowId, initialDraft, latestVersionId, agents = [], agentVersions = [], tools = [] }: { workflowId: string; initialDraft: WorkflowDraft; latestVersionId?: string | null; agents?: Agent[]; agentVersions?: AgentVersionSummary[]; tools?: Tool[] }) {
